...
from forms import CreatePostForm, RegisterForm, SigninFrom, CommentForm
...
def add_new_post():
    form = CreatePostForm()
    if form.validate_on_submit():
        print(form.title.data, form.subtitle.data, form.body.data, form.img_url.data, current_user.name, date.today().strftime("%B %d, %Y"))
        new_post = BlogPost(
